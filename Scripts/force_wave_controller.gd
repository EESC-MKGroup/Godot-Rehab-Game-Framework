...
remote func update_server( input_wave, remote_position, remote_force, client_time, last_server_time ):
	...
	print( feedback_force, "=-", wave_impedance, "*", local_velocity, "+sqrt(2*", wave_impedance, ")*", filtered_wave )
	...
	print( output_wave, "=(", wave_impedance, "*", local_velocity, "-", feedback_force, "/sqrt(2*", wave_impedance, ")" )
